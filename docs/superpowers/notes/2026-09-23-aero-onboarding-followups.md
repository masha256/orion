...
## The first signal (checkpoint 4, 2026-09-24)

`orion update aero` after the calibration: `aero-20260924T015415Z-11`, status ok, grade B, spot 0.667557; 12m expected 0.382 (bear 0.047 / base 0.247 / bull 0.987, dispersion 0.85, upside -42.8 percent); 6m expected 0.352 (dispersion 0.74); 90-day run rate 61.77M USD (observation #530). Signals 9 and 10 that morning carried the spike (12m expected 0.574) and are superseded by 11. The valuation says the market pays roughly the constructive package's base case; the calibrated base case sits well under spot, as the VVV one does. The weekly agent run is due from the tick cadence (no set existed before 2026-09-24, so the first scheduled run was the bootstrap).

Ruling (2026-09-24, after the merge): the derived step read only onchain and api daily rows, so the manual replacement of the 2026-09-09 day left a hole in every 90-day window; `revenue_run_rate_usd` reported "no new day with 90 complete days behind it" and would have stayed at 92.8M USD until 2026-12-08. Fixed on main (spec amendment 8): `storedDailyFlow` takes every active daily row whatever its source, with a test. Cost if wrong: a manual daily row entered by mistake enters the run rate, as a manual row of any other metric already would.
