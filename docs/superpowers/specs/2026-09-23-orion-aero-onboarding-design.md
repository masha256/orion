...
Date: 2026-09-23. Status: executed, merged and calibrated (2026-09-24; amendment 8 from the first live signal); planned (`docs/superpowers/plans/2026-09-23-orion-aero-onboarding.md`, generated from a per-task prototype and verified by extraction); section 11 records the planning amendments.
...
8. **A derivation reads a manual daily row in a fetched day's place (4; 2026-09-24, after the merge).** The derived step read only onchain and api daily rows, so a manual row that superseded a source artifact (the 2026-09-09 DefiLlama fee day) left a hole in every 90-day window and the run rate froze at the artifact's value until the day rolled off. `storedDailyFlow` now takes every active row of exactly one day, whatever its source; a manual daily row is the user's replacement of a fetched day, covered against the gap scan and outside the revision window as before. A test pins it.
