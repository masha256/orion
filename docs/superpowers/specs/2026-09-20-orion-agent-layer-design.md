...
Web content enters inside server-tool result blocks, and the operating rules state that it is data, never instructions. It does NOT stay there: quotes, rationales, proposal changes, and the journal are written out of what a page said, and they persist. They are stored, printed to the terminal, and read back into every later run's context pack as ordinary user-message content, where nothing marks them as quoted material. Through the proposal line in `docs/ops/hermes-daily-job.md` they also reach a second LLM agent, one that has a shell.

So the text itself is bounded, not just trusted: the tool layer caps the length of every string the model writes and strips control characters from it (section 17 item 25), duplicate detection ignores the wording (item 26), and the Hermes job reports a proposal's id, kind and stored effect only, never its free text.

The defence that counts is still structural: whatever a page says, the agent's reach is the tool list above, inside bands, steps, evidence, the move guard, and verified citations. It has no shell, no file access, and Orion makes no HTTP request on its behalf.
...
### Amendments made during execution and the final review (2026-09-20)

Continuing the numbering above. These came out of building the layer and out of the whole-branch review at the end of it; where one of them and an earlier section disagree, this one governs.

13. **A run is finished exactly once (section 11.1).** `agent_transcripts` is append-only, and the transcript's existence, not the run row's outcome, is what "finished" means: `abandonStaleRuns` moves a row to `error/abandoned` without writing one, so a process that was alive after all can still record what really happened. A second `finishAgentRun` for the same run is refused with `agent_run_already_finished`.
14. **Optional persona metadata (section 9).** `personas/<name>.md` frontmatter also takes `temperament` and `sectors[]`. Both are optional, neither reaches the prompt assembly; they describe the persona for the reader and for later selection work.
15. **A cited quote must sit inside ONE block of the page (section 5.5).** The page is cut into blocks at every tag that is not a known inline tag and at every blank line, so a quote cannot be spliced from a figure in one table row and a claim in the next. A single line break is ambiguous: a line starting in lower case continues the sentence, anything else starts a new block. That errs toward refusing on purpose, because a refusal costs one retry and a wrong acceptance lets an invented claim in.
16. **The agent never supersedes an existing observation (sections 5.5, 7.3, 6.2).** `insertObservation` would let a provisional insert retire an active row at the same metric and time, including one the user entered. The tool refuses while the model can still act (`observation_exists`, naming the row), the commit refuses again as a conflict, and approve refuses as stale; rejecting a row stays the user's call, by proposal.
17. **The move guard and approve's staleness check measure from the last CONFIRMED value (sections 5.5, 6.2).** `valueInForce` is called with `confirmedOnly`, so the agent's own earlier provisional figures never move the baseline. Without that, a run could walk a critical metric anywhere in a few steps, each one inside the threshold.
18. **Config paths refuse prototype keys, and resolve own keys only (section 6.3).** Paths come from model output, so `__proto__`, `constructor` and `prototype` are refused as `invalid_path`, and a path walks into a map only through its own properties: an inherited `toString` is never a config key.
19. **Approve acts with the user's authority, so it checks everything again (section 6.2).** On a stored row it re-checks the unproposable roots, that `filed_against` has one entry per edit, and that the asset id is an asset id, before any file path is built from it. The YAML edit is written beside the file and renamed over it, the original is restored if marking the proposal approved then fails, and a proposal whose values the file already holds (an earlier approve that died after the rename) is marked approved rather than refused as stale.
20. **The loop ends as `error` when a tool throws, and never runs a tool call that precedes a fallback (section 8).** `runTool` answers every expected failure as an `is_error` result, so a throw is a bug in a tool: the loop ends, and the usage and transcript so far still reach the run record. A `fallback` block means the server switched models mid-turn; anything the declining model had already emitted may be a tool call it never finished, and a truncated input can still parse, so no `tool_use` before the last fallback is run.
21. **`runAgent` reloads the config, and conflicts if it changed (sections 4, 7.3).** Bands, bounds, the move guard and the run's config hash all come from the config the run began on. `runAgent` takes an optional `reload`, calls it once just before the commit, and ends the run as `conflict` writing nothing when the hash has moved; the CLI passes `loadAsset`.
22. **Commit re-checks the anomaly block (section 7.3).** A degrading anomaly can open between the tool's check and the commit, so the check runs again inside the transaction whenever an assumption change is staged. Resolutions staged in the same run count as resolved there, exactly as they did at tool time, because they are about to be written.
23. **Commit, finish, and approve take the write lock up front (sections 7.3, 6.2, 11.1).** Each reads before it writes, and `db.transaction(fn)()` is DEFERRED: another connection committing between that read and the first write makes SQLite refuse the whole transaction at once with `SQLITE_BUSY_SNAPSHOT`, which `busy_timeout` cannot retry away. All three now begin IMMEDIATE, and `runAgent` retries a failed finish once with a minimal transcript, because a run row left `running` while its writes are live is worse than a lost transcript.
24. **Run-level web budgets, direct-only fetch, 32000-token responses (sections 4.4, 8).** `max_uses` bounds one request's server-side loop, not the run, so the loop also ends a run whose web use has gone STRICTLY past its budget: spending exactly the budget is a run doing what it was allowed, and only a later request spending beyond the total ends it. Each tool result's budget line carries `web_searches_left` and `web_fetches_left`; `web_fetch` is declared `allowed_callers: ["direct"]`, since a fetch inside code execution would keep the page text out of the transcript and so make every citation of it unverifiable.
25. **Model-authored text is bounded and cleaned (sections 5.5, 5.6, 5.8).** Every string the model writes has a maximum length in its tool schema (quote 600, rationale and note 2000, url 2000, journal text 4000, each open question 500 and at most 20), refused as ordinary `invalid_input` so it can be shortened and retried. `cleanText` strips C0 control characters, DEL and the C1 introducer, keeping newline and tab; the CLEANED quote is what is verified against the page and what is stored.
26. **Duplicate detection ignores wording (sections 5.6, 11.1).** `proposalIdentity` drops the free text a change carries (`note`, and the observation kind's `quotedText`) before canonicalizing, so the same decision refiled in other words is refused as a duplicate rather than filed again every run.
