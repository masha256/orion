...
Date: 2026-09-21. Status: implemented on branch feat/scheduling (2026-09-21/22); section 15 records every amendment from planning, execution, and the final review.
...
### Amendments made during execution and the final review (2026-09-21)

Each task was reviewed against this spec and the plan's named risks; the final whole-branch review attacked cross-task interactions and the real clock. What changed:

- **7, anchor (design change, ruled at the final review).** Under the default cadence a completed weekly reset the anchor every 7 days and, with the 7-day minimum age, `driver_deviation` could never exceed 25 percent: it was inert. The anchor is now the CURRENT ASSUMPTION SET's creation: the path is the set's, a review that changed nothing does not reset the clock, and an assumption change (the agent's or the user's) creates a new set and moves the anchor. Decision 5's "anchored at the last agent review" reads "anchored at the current assumption set". `revenueAnchor(db, asset)` takes no `now`; `from` is always `assumption_set`.
- **7, year length.** Elapsed years use the engine's `DAYS_PER_YEAR` (365), not 365.25.
- **5.2 and 7, boundaries.** Both ends of an interval are read after fetches of different length, so a boundary at exactly N days is decided by jitter. A run is due when the elapsed time is at least the interval minus half a day; the anchor must be at least 6.5 days old.
- **6.4, deviation guard.** A revenue that is stale is never measured, critical or not (`staleMetrics`, not `staleCritical`); `report.drivers === null` (any required metric missing) also skips the check.
- **6.1.** Calendar dates must be real dates (a round-trip refine) and unique per asset (a parse-time error; two notes on one date go in one note).
- **6.3.** `staleness.newest_observed_at` is the newest ELIGIBLE row's date (the rows the verdict came from). A `calendar` firing's detail carries the note in the row and the pack; the REPORT's copy is `{}` (the note is the user's YAML, but the report promises no free text).
- **6.5.** The pack's `triggers_this_tick` entries carry `fired_at`. `skills/assumption-review.md` gained one sentence about the block.
- **4.4, report.** New field `triggers_standing: [{ kind, key, agent_run_id }]`: instances that fired on an earlier tick and still hold; `agent_run_id` null means no run ever got a row for it. `error.message` and `agent.error.message` are capped at 300 characters; `error.code` for a non-Orion throw is the constructor name. A committed run whose revaluation failed reports `agent.error = { code: 'valuation_error' }` with `outcome: completed`.
- **4.2, retry of unhandled firings.** Standing firings with `agent_run_id` null (a preflight failure: no credentials, no persona) join the next tick's firings for the run choice and `attachRun`, so a preflight failure is retried daily at no spend; a firing whose run got a row and failed keeps its run id and waits (decision 1). Spec 14's first limitation is withdrawn.
- **4.2, delivery.** `agentStage` records the run's fields before delivering its signal; both `onSignal` deliveries and the `ticks.jsonl` append are guarded (a warning on stderr, never a crash; the line on stdout is the delivery, the files are copies, the rows are in the database); `attachRun` is guarded. The trigger and cadence clock is `deps.now()` read after the fetch, not the valuation's `asOf`. `tickAsset(db, loaded, deps, opts)`: `now` is in `TickDeps`. tick's stderr `signalSummary` omits `change.rationale` (model text); the Hermes prompt's never-relay list names the assumption-change rationale.
- **8.2, lock.** `withRunLock(..., { onTakeover, onReleaseError })`: a release failure never masks `fn`'s outcome and is reported through the callback; `acquireRunLock` returns `{ abandoned, tookOver }` and every takeover is reported, even with 0 abandoned; `attachRun` runs in one transaction and returns the count; after `run_in_progress` tick reads the lock row and falls back to `holder: 'unknown'`.
- **11.** Exit 1 carries an error report on stdout (spec 11's "empty on exit 1" is superseded; stdout is empty only when the command failed before `tickAsset`). The Hermes prompt: exit 1 is retried once with or without a report; `run_in_progress` is retried once after 30 minutes; the job timeout is 150 minutes, past the lock TTL; the read-only list keeps `model proposals list` (the always-on step went, the on-demand command stays).
- **14.** Budgets bound requests and tokens, not time (the "none can outlive 2 h" claim is withdrawn); a `running` row with no lock row is never abandoned (the takeover is the only sweep).
- **README, vvv hash pin.** `tests/app/cadence.test.ts` pins the config hashes of the mini fixture and `assets/vvv.yaml` as measured on `main` before this branch: a deliberate edit of `vvv.yaml` (an approved config proposal included) moves the second pin, which is then updated on purpose.
